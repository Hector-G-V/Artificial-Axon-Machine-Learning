...
def neural_network_model(x, n_nodes_hl, n_classes):
    ...
    hl_matrices = {"weights": tf.Variable(tf.random_uniform([784, n_nodes_hl], minval=-0.1, maxval=0.1)),
                   "biases": tf.Variable(tf.random_uniform([n_nodes_hl], minval=-0.1, maxval=0.1))}
    # Output layer matrices
    ol_matrices = {"weights": tf.Variable(tf.random_uniform([n_nodes_hl, n_classes], minval=-0.1, maxval=0.1)),
                   "biases": tf.Variable(tf.random_uniform([n_classes], minval=-0.1, maxval=0.1))}
