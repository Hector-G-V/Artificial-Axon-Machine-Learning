...
def neural_network_model(x, n_nodes_hl, n_classes):
    ...
    hl_matrices = {"weights": tf.Variable(tf.random_normal([784, n_nodes_hl])),
                   "biases": tf.Variable(tf.random_normal([n_nodes_hl]))}
    # Output layer matrices
    ol_matrices = {"weights": tf.Variable(tf.random_normal([n_nodes_hl, n_classes])),
...
if __name__ == '__main__':
    ...
    # Args
    n_nodes = 5  # Number of hidden layer nodes.
