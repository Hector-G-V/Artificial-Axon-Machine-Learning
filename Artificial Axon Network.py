...
def network(y, t):
    ...
    for h in range(len(nodes)-1):
        ...
        for j in range(nodes[h]):
            k = 2*(sum(nodes[:h]) + j)
            a_0 = np.append(a_0, y[k]*np.heaviside(y[k], 0))
        a_1 = np.matmul(syn[h], a_0)  # a_1 layer vector.
        Syn = np.append(Syn, a_1)

    Syn = np.insert(Syn, 0, np.zeros(nodes[0]))  # Input to each node.
    Syn[0] = start_pulse(t)  # The first node is the initialization node. It initializes the input layer.
...
def plot(solution, t):
    ...
    layer = ['Activation Layer', 'Input Layer']
...
if __name__ == '__main__':
    # Args.
    t_i, t_f = 0, 4  # Simulation start and end times.

    # Synapse initialization. Will need to put into a function later. # 0.767501, 0.772620
    Rs = 5 * 10 ** 8
    syn = [0.8*np.ones(3).reshape(3, 1)/Rs, np.array([[0.5, 0.1, 0.1], [0.6, 0.1, 0.1]])/Rs, np.array([[0.5, 0.3]])/Rs, np.array([[0.8]])/Rs]
    nodes = node_counts(syn)

    # Solve.
    y0 = p_inf(int(sum(nodes)))  # Initial Values. All units are SI.
    t_steps = np.linspace(t_i, t_f, 10001)  # Time steps.
    sol = odeint(network, y0, t_steps)  # Solves the system of equations. "NODE" WILL BE REPLACED WITH "EQUATIONS."
